...
- An offline snapshot at `as_of` may use only profile mutations with
  `occurredAt <= as_of * 1000` and events with `event.time <= as_of`. Training features use a cutoff
  strictly earlier than the first label, so label events cannot enter their own features.
